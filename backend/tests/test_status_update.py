...
class TestStatusUpdateCSVPersistence:
    ...
    def temp_csv_dir(self, monkeypatch):
        ...
        with open(csv_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                "INC001,Test Incident,Test Description,high,open,2026-01-17T10:30:00,,service1|service2,ops-team\n"
            )
            f.write(
                "INC002,Another Incident,Another Description,medium,investigating,2026-01-17T12:00:00,,service3,\n"
            )
    ...
    def test_save_incidents_preserves_other_incidents(self, temp_csv_dir, monkeypatch):
        ...
        inc002_original = next(
            inc for inc in test_incidents if inc["id"] == "INC002"
        ).copy()
...
class TestStatusUpdateAPI:
    ...
    def test_update_status_endpoint_success(self, test_client):
        ...
        response = test_client.put(
            "/api/v1/incidents/INC001/status", json={"status": new_status}
        )
        assert response.status_code == 200
    ...
    def test_update_status_endpoint_invalid_status(self, test_client):
        ...
        response = test_client.put(
            "/api/v1/incidents/INC001/status", json={"status": "invalid_status"}
        )
    ...
    def test_update_status_endpoint_not_found(self, test_client):
        """Test status update for non-existent incident."""
        response = test_client.put(
            "/api/v1/incidents/INC999/status", json={"status": "resolved"}
        )
    ...
    def test_update_status_endpoint_valid_statuses(self, test_client):
        ...
        for status in valid_statuses:
            response = test_client.put(
                "/api/v1/incidents/INC001/status", json={"status": status}
            )
            assert response.status_code == 200
