    ...
    try:
        ...
        with open(csv_path, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                affected_services = (
                    row["affected_services"].split("|")
                    if row["affected_services"]
                    else []
                )
...
def _load_servicenow_tickets() -> dict[str, list[dict[str, Any]]]:
    ...
    try:
        ...
        raise MockDataLoadError(
            f"Error loading ServiceNow tickets CSV: {str(e)}"
        ) from e
...
def _load_change_correlations() -> dict[str, list[dict[str, Any]]]:
    ...
    try:
        ...
        raise MockDataLoadError(
            f"Error loading change correlations CSV: {str(e)}"
        ) from e
            ...
            for incident in incidents:
                ...
                if incident.get("updated_at"):
                    updated_at_str = (
                        ...
                        if isinstance(incident["updated_at"], datetime)
    ...
    for incident_id, tickets in MOCK_SERVICENOW_TICKETS.items():
        for ticket in tickets:
            ...
            if incident_id not in incident_ids:
                errors.append(
                    f"Ticket {ticket_id}: References non-existent incident {incident_id}"
                )
                invalid_tickets += 1
            ...
            if ticket_type not in valid_ticket_types:
                errors.append(
                    f"Ticket {ticket_id}: Invalid type '{ticket_type}', must be one of {valid_ticket_types}"
                )
                invalid_tickets += 1
            ...
            if source not in valid_sources:
                errors.append(
                    f"Ticket {ticket_id}: Invalid source '{source}', must be one of {valid_sources}"
                )
                invalid_tickets += 1
            ...
            if ticket_type == "similar_incident":
                if not resolution:
                    warnings.append(
                        f"Ticket {ticket_id}: Similar incident ticket missing resolution field"
                    )
                if description:
                    warnings.append(
                        f"Ticket {ticket_id}: Similar incident ticket has description field (should be empty)"
                    )
            elif ticket_type == "related_change":
                if not description:
                    warnings.append(
                        f"Ticket {ticket_id}: Related change ticket missing description field"
                    )
                if resolution:
                    warnings.append(
                        f"Ticket {ticket_id}: Related change ticket has resolution field (should be empty)"
                    )
...
try:
    ...
    if validation_results["errors"]:
        ...
        print(
            f"ERROR: ServiceNow ticket validation failed with {len(validation_results['errors'])} error(s):",
            file=sys.stderr,
        )
    ...
    if validation_results["warnings"]:
        ...
        print(
            f"WARNING: ServiceNow ticket validation found {len(validation_results['warnings'])} warning(s):",
            file=sys.stderr,
        )
        ...
        if len(validation_results["warnings"]) > MAX_WARNINGS_TO_DISPLAY:
            print(
                f"  ... and {len(validation_results['warnings']) - MAX_WARNINGS_TO_DISPLAY} more warnings",
                file=sys.stderr,
            )
