...
class OrchestratorAgent:
    ...
    async def _query_servicenow(self, state: IncidentState) -> IncidentState:
        ...
        logger.debug(
            f"ServiceNow query complete",
            extra={
                "extra_fields": {
                    "similar_incidents_count": len(results.get("similar_incidents", [])),
                    "related_changes_count": len(results.get("related_changes", []))
                }
            }
        )
        return state
    ...
    async def _query_changes(self, state: IncidentState) -> IncidentState:
        ...
        logger.info(f"Querying change correlations for incident: {state['incident_id']}")
        ...
        logger.debug(
            f"Change correlation query complete",
            extra={
                "extra_fields": {
                    "high_correlation_count": len(results.get("high_correlation_changes", [])),
                    "medium_correlation_count": len(results.get("medium_correlation_changes", [])),
                    "top_suspect": results.get("top_suspect", {}).get("change_id") if results.get("top_suspect") else None
                }
            }
        )
        return state
    ...
    async def _synthesize_results(self, state: IncidentState) -> IncidentState:
        ...
        logger.debug(
            ...
            extra={
                "extra_fields": {
                    "has_top_suspect": top_suspect is not None,
        ...
        logger.info(
            ...
            extra={
                "extra_fields": {
                    ...
                    "resolution_steps_count": len(resolution_steps),
                    "correlated_changes_count": len(correlated_changes)
                }
            }
        )
...
4. Is clear and actionable for the incident responder""")
        ...
        try:
            ...
            logger.warning(f"LLM invocation failed, using fallback summary: {str(e)}")
    ...
    async def resolve(self, incident_id: str, user_query: str) -> AgentResponse:
        ...
        logger.info(
            f"Incident resolution workflow complete",
            extra={
                "extra_fields": {
                    "incident_id": incident_id
                }
