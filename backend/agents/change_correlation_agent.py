...
class ChangeCorrelationAgent:
    ...
    async def query(self, incident_id: str, context: str) -> dict[str, Any]:
        ...
        high_correlation = [
            c for c in correlations if c.get("correlation_score", 0) >= 0.8
        ]
        medium_correlation = [
            c for c in correlations if 0.5 <= c.get("correlation_score", 0) < 0.8
        ]

        logger.debug(
            f"Found {len(high_correlation)} high and {len(medium_correlation)} medium correlation changes"
        )
