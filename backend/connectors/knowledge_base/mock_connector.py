...
class MockKnowledgeBaseConnector(KnowledgeBaseConnectorBase):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        logger.debug(
            f"MockKnowledgeBaseConnector initialized with csv_path={self.csv_path}, docs_folder={self.docs_folder}"
        )
    ...
    def _load_text_documents(self):
        ...
        if not docs_path.is_absolute():
            ...
            if backend_dir:
                ...
                docs_path = (
                    Path(__file__).parent.parent.parent.parent / self.docs_folder
                )
        ...
        logger.info(
            f"Loaded {len(self.text_documents)} text documents from {docs_path}"
        )
