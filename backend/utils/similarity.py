...
def calculate_incident_similarity(
    incident1: dict[str, Any], incident2: dict[str, Any]
) -> float:
    """
    ...
    title_sim = calculate_text_similarity(
        incident1.get("title", ""), incident2.get("title", "")
    )

    desc_sim = calculate_text_similarity(
        incident1.get("description", ""), incident2.get("description", "")
    )

    service_sim = calculate_service_similarity(
        incident1.get("affected_services", []), incident2.get("affected_services", [])
    )

    # Weighted average
    # Title and description are more important than services
    weights = {"title": 0.4, "description": 0.4, "services": 0.2}
