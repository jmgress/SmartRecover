...
class ConnectorConfig(BaseModel):
    ...
    mock: MockConfig | None = Field(default_factory=MockConfig)
...
class KnowledgeBaseConfig(BaseModel):
    ...
    confluence: ConfluenceKBConfig | None = Field(default_factory=ConfluenceKBConfig)
    mock: MockKBConfig | None = Field(default_factory=MockKBConfig)
...
def load_config_from_env() -> ConnectorConfig:
    ...
    if connector_type == "servicenow":
        ...
        # Default to mock
        connector_type = "mock"
        mock_config = MockConfig(data_source=os.getenv("MOCK_DATA_SOURCE", "mock"))
...
class ConfigManager:
    ...
    def _load_config(self) -> Config:
        ...
        if enable_tracing:
            config_dict["logging"]["enable_tracing"] = enable_tracing.lower() in (
                "true",
                "1",
                "yes",
            )
        ...
        if confluence_base_url:
            config_dict["knowledge_base"]["confluence"]["base_url"] = (
                confluence_base_url
            )

        confluence_username = os.getenv("CONFLUENCE_USERNAME")
        if confluence_username:
            config_dict["knowledge_base"]["confluence"]["username"] = (
                confluence_username
            )

        confluence_api_token = os.getenv("CONFLUENCE_API_TOKEN")
        if confluence_api_token:
            config_dict["knowledge_base"]["confluence"]["api_token"] = (
                confluence_api_token
            )

        confluence_space_keys = os.getenv("CONFLUENCE_SPACE_KEYS")
        if confluence_space_keys:
            config_dict["knowledge_base"]["confluence"]["space_keys"] = (
                confluence_space_keys.split(",")
            )
    ...
    def update_logging_config(
        self, level: str | None = None, enable_tracing: bool | None = None
    ):
