...
async def update_incident_status_endpoint(
    incident_id: str, request: UpdateStatusRequest
):
    ...
    try:
        ...
        for inc in mock_data.MOCK_INCIDENTS:
            if inc["id"] == incident_id:
                logger.info(
                    f"Successfully updated incident {incident_id} status to {request.status}"
                )
                return Incident(**inc)

        # This should not happen, but handle it just in case
        raise HTTPException(
            status_code=500, detail="Failed to retrieve updated incident"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update incident status: {str(e)}"
        )
...
async def get_incident_details(incident_id: str):
    ...
    return {"incident": incident_data, "agent_results": agent_results}
...
async def retrieve_incident_context(incident_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve agent context: {str(e)}"
        )
...
async def resolve_incident(query: IncidentQuery):
    ...
    logger.info(
        f"Resolving incident: {query.incident_id} with query: {query.user_query}"
    )
    incident_exists = any(
        inc["id"] == query.incident_id for inc in mock_data.MOCK_INCIDENTS
    )
    if not incident_exists:
    ...
    logger.info(
        f"Incident resolution complete: {query.incident_id}, confidence: {response.confidence}"
    )
    return response
...
async def test_llm(request: LLMTestRequest):
    ...
    try:
        ...
        return LLMTestResponse(status="success", llm_response=response.content)
    except Exception as e:
        logger.error(f"LLM test failed: {str(e)}")

        return LLMTestResponse(status="error", llm_response="", error=str(e))
    ...
    if llm_config.provider == "openai":
        ...
        connection_details = {
            "api_key_configured": bool(
                llm_config.openai.api_key or os.getenv("OPENAI_API_KEY")
            ),
        ...
        connection_details = {
            "api_key_configured": bool(
                llm_config.gemini.api_key or os.getenv("GOOGLE_API_KEY")
            ),
        ...
        connection_details = {"base_url": llm_config.ollama.base_url, "local": True}
    else:
        model = "unknown"
        connection_details = {"error": "Unknown provider"}

    logger.info(
        f"LLM configuration retrieved: provider={llm_config.provider}, model={model}"
    )
...
async def update_logging_config(request: UpdateLoggingConfigRequest):
    ...
    logger.info(
        f"Updating logging configuration: level={request.level}, enable_tracing={request.enable_tracing}"
    )
    ...
    try:
        ...
        config_manager.update_logging_config(
            level=request.level, enable_tracing=request.enable_tracing
        )

        # Return updated configuration
        updated_config = config_manager.get_logging_config()
        logger.info(
            f"Logging configuration updated successfully: level={updated_config.level}, tracing={updated_config.enable_tracing}"
        )
...
async def chat_stream(request: ChatRequest):
    ...
    # Verify incident exists
    incident_exists = any(
        inc["id"] == request.incident_id for inc in mock_data.MOCK_INCIDENTS
    )
    if not incident_exists:
    ...
    async def generate_stream():
        ...
        try:
            async for chunk in orchestrator.chat_stream(
                request.incident_id, request.message, request.conversation_history
            ):
                # Format as SSE event
                yield f"data: {chunk}\n\n"
